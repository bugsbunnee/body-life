...
router.post('/bulk', [auth, protectedRoute(DEFAULT_ROLES), upload.single('file')], userController.bulkCreateUsers);
...
router.put('/:id', [auth, protectedRoute(DEFAULT_ROLES), validateObjectId, validate(UserUpdateSchema, 'body')], userController.updateUser);
...
router.post('/', [auth, protectedRoute(DEFAULT_ROLES), validate(UserCreationSchema, 'body')], userController.createUser);
router.get('/', [auth, protectedRoute(DEFAULT_ROLES), validate(UserQuerySchema, 'query'), paginate], userController.getUsers);
